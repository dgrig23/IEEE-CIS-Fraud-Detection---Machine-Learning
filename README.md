...
LR-ისთვის მკაცრი ზღვარი (0.5) გამოვიყენე, რადგან linear მოდელი sparse სვეტებს ვერ ეფექტურად ამუშავებს და noise-ი ზიანს მიაყენებდა. Tree-based მოდელები კი `fillna(-999)` ან median-ით უმკლავდებიან missing values-ს, ამიტომ 0.9 ზღვარი საკმარისია.
...
| `dayofweek` | კვირის დღე (0–6) | შვებულება vs სამუშაო დღე განსხვავდება |
...
→ L1 penalty-ით SelectFromModel გამარჯვდა. L1 regularization linear მოდელისთვის ბუნებრივი feature selector-ია — არარელევანტური coefficients-ები ნულამდე მიჰყავს.
...
→ VarianceThreshold გამარჯვდა ოდნავ. AUC-ები ძალიან ახლოსაა, რაც ნიშნავს Decision Tree-ს feature selection-ისადმი სენსიტიური არ არის — tree-ები არასაჭირო features-ებს ისედაც უგულებელყოფს.
...
→ Correlation filter გამარჯვდა. 60 features-მდე შეკვეცა ოდნავ ამცირებს AUC-ს, რაც ნიშნავს რომ RF-ს 360 feature-ის კარგად გარჩევა შეუძლია parallel ხეებით.
...
→ ყველა feature-ის გამოყენება ან VarianceThreshold გამარჯვდა (AUC იდენტურია). AdaBoost weak learners-ს (depth-1 stump) იყენებს, ამიტომ ბევრი feature-ი redundant-ია — ამ შემთხვევაში selection ეფექტი მინიმალურია.
...
→ XGBoost Importance-based selection გამარჯვდა. importance-ის მიხედვით top-150 feature-ის შერჩევა noise-ს ამცირებს და უკეთ განაზოგადებს.
...
ყველა მოდელში განზრახ შევქმენი underfitting და overfitting კონფიგურაციები bias-variance tradeoff-ის სადემონსტრაციოდ:
...
| | max_depth=None (overfit) | 1.00000 | 0.76063 | 0.239 | depth=53 — ყველა train სტრიქონს ზეპირობს |
| **Random Forest** | n=10, depth=3 (underfit) | 0.80548 | 0.80895 | ~0 | 10 shallow tree საკმარისი არ არის |
| | n=200, depth=None, 5% (overfit) | 1.00000 | 0.88965 | 0.110 | unlimited depth 5%-ზე — memorization |
...
| | n=700, depth=12, reg=0 (overfit) | 1.00000 | 0.96374 | 0.036 | regularization-ის გარეშე სრული memorization |

**დასკვნები:**
- **LR** overfitting-ს ძალიან სწრაფად აღწევს (500 სტრიქონი + C=1e9 → gap=0.44) — linear მოდელი ადვილად ზეპირობს მცირე dataset-ს
- **Decision Tree** ყველაზე დრამატული overfit-ი აქვს (gap=0.239) — regularization-ის გარეშე ხე depth=53-მდე იზრდება
- **Random Forest** ზომიერ overfit-ს აჩვენებს (gap=0.11) — ensemble averaging ეხმარება, მაგრამ depth=None 5% dataset-ზე მაინც memorizes
- **AdaBoost** ბუნებრივად ძნელად მიდის overfit-ში (ensemble) მაგრამ მცირე dataset-ზე + aggressive lr მაინც გამოიწვია (gap=0.12)
- **XGBoost** regularization-ის (reg_alpha, reg_lambda) გარეშე overfit-ს აღწევს (gap=0.036), მაგრამ regularization-ით კარგი ბალანსი ინარჩუნებს
...
| Random Forest | 0.861 | `RandomForest_FraudDetection` | ✅ |
...
| მოდელი | სახელი | ვერსია |
|---|---|---|
| XGBoost | `XGBoost_FraudDetection` | v1 |
| AdaBoost | `AdaBoost_FraudDetection` | v1 |
| Random Forest | `RandomForest_FraudDetection` | v1 |
| Decision Tree | `DecisionTree_FraudDetection` | v2 |
| Logistic Regression | `LogisticRegression_FraudDetection` | v1 |
...
## გამოსაგენერირებელი გრაფიკები

**⚠️ ქვემოთ მოყვანილი კოდის ბლოკები დაამატე შენს notebooks-ში `images/` ფოლდერში შესანახად. ეს სურათები README-ში ჩასმული იქნება.**

### 1. Bias-Variance Comparison (`images/bias_variance_comparison.png`)

დაამატე **ნებისმიერ ერთ notebook-ში** (მაგ. XGBoost-ში) ბოლო cell-ად:
...
train_aucs = [1.000, 1.000, 1.000, 1.000, 1.000]
val_aucs   = [0.556, 0.761, 0.890, 0.879, 0.964]
underfit_train = [0.759, 0.628, 0.805, 0.751, 0.875]
underfit_val   = [0.763, 0.628, 0.809, 0.755, 0.873]

x = range(len(models))
fig, axes = plt.subplots(1, 2, figsize=(14, 5))

# Overfit configs
axes[0].bar([i - 0.2 for i in x], train_aucs, width=0.4, label='Train AUC', color='#e74c3c', alpha=0.8)
axes[0].bar([i + 0.2 for i in x], val_aucs,   width=0.4, label='Val AUC',   color='#3498db', alpha=0.8)
axes[0].set_xticks(x); axes[0].set_xticklabels(models)
axes[0].set_ylim(0.5, 1.05); axes[0].set_title('Overfit Configs — Train vs Val AUC')
axes[0].set_ylabel('ROC-AUC'); axes[0].legend(); axes[0].grid(axis='y', alpha=0.3)

# Underfit configs
axes[1].bar([i - 0.2 for i in x], underfit_train, width=0.4, label='Train AUC', color='#e74c3c', alpha=0.8)
axes[1].bar([i + 0.2 for i in x], underfit_val,   width=0.4, label='Val AUC',   color='#3498db', alpha=0.8)
axes[1].set_xticks(x); axes[1].set_xticklabels(models)
axes[1].set_ylim(0.5, 1.05); axes[1].set_title('Underfit Configs — Train vs Val AUC')
axes[1].set_ylabel('ROC-AUC'); axes[1].legend(); axes[1].grid(axis='y', alpha=0.3)

plt.suptitle('Bias-Variance Analysis per Model', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.savefig('images/bias_variance_comparison.png', dpi=150, bbox_inches='tight')
plt.show()
print('Saved: images/bias_variance_comparison.png')
```

### 2. Model AUC Comparison (`images/model_auc_comparison.png`)

დაამატე `model-inference.ipynb`-ში predictions-ის შემდეგ:
...
models  = ['Logistic\nRegression', 'Decision\nTree', 'Random\nForest', 'AdaBoost', 'XGBoost']
val_auc = [0.830, 0.856, 0.861, 0.867, 0.970]
colors  = ['#95a5a6','#27ae60','#2980b9','#e67e22','#e74c3c']

fig, ax = plt.subplots(figsize=(10, 5))
bars = ax.bar(models, val_auc, color=colors, alpha=0.85, edgecolor='white', linewidth=1.2)
ax.set_ylim(0.75, 1.00)
ax.set_ylabel('ROC-AUC (Val / OOF)', fontsize=12)
ax.set_title('Final Model Comparison — IEEE-CIS Fraud Detection', fontsize=13, fontweight='bold')
ax.axhline(y=0.9, color='gray', linestyle='--', alpha=0.5, label='AUC=0.90 reference')
for bar, v in zip(bars, val_auc):
    ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.003,
            f'{v:.3f}', ha='center', fontsize=11, fontweight='bold')
ax.legend(); ax.grid(axis='y', alpha=0.3)
plt.tight_layout()
plt.savefig('images/model_auc_comparison.png', dpi=150, bbox_inches='tight')
plt.show()
print('Saved: images/model_auc_comparison.png')
```

### 3. XGBoost Feature Importance (`images/feature_importance_xgboost.png`)

დაამატე `model-experiment-xgboost.ipynb`-ში Final Model cell-ის შემდეგ:
...
# final_pipeline['preprocessor'] + final_pipeline['classifier']
clf = final_pipeline.named_steps['classifier']
prep_features = final_features  # top_150 selected features

imp = pd.Series(clf.feature_importances_, index=prep_features).nlargest(25)

fig, ax = plt.subplots(figsize=(10, 8))
imp.sort_values().plot(kind='barh', ax=ax, color='#e74c3c', alpha=0.8)
ax.set_title('XGBoost — Top 25 Feature Importances', fontsize=13, fontweight='bold')
ax.set_xlabel('Importance Score')
ax.grid(axis='x', alpha=0.3)
plt.tight_layout()
plt.savefig('images/feature_importance_xgboost.png', dpi=150, bbox_inches='tight')
plt.show()
print('Saved: images/feature_importance_xgboost.png')
```

### 4. Feature Selection Strategy Comparison (`images/feature_selection_comparison.png`)

დაამატე **ნებისმიერ ერთ notebook-ში** Feature Selection section-ის შემდეგ:
...
strategies = {
    'LR':  {'labels':['All','VT','Correlation','L1 Lasso'],
             'aucs':[0.82812, 0.82812, 0.81923, 0.83003]},
    'DT':  {'labels':['All','VT','Correlation','Top-100 Imp.'],
             'aucs':[0.85845, 0.85872, 0.85855, 0.85815]},
    'RF':  {'labels':['Correlation','RF Imp. top-60'],
             'aucs':[0.86117, 0.85933]},
    'Ada': {'labels':['All','VT','Correlation'],
             'aucs':[0.86663, 0.86663, 0.85966]},
    'XGB': {'labels':['All','VT','XGB Imp. top-150'],
             'aucs':[0.90751, 0.90751, 0.91213]},
}

fig, axes = plt.subplots(1, 5, figsize=(18, 5))
for ax, (name, data) in zip(axes, strategies.items()):
    colors = ['#e74c3c' if v == max(data['aucs']) else '#3498db' for v in data['aucs']]
    ax.bar(range(len(data['labels'])), data['aucs'], color=colors, alpha=0.8)
    ax.set_xticks(range(len(data['labels'])))
    ax.set_xticklabels(data['labels'], rotation=30, ha='right', fontsize=8)
    ax.set_title(name, fontweight='bold')
    ax.set_ylim(min(data['aucs']) - 0.01, max(data['aucs']) + 0.01)
    for i, v in enumerate(data['aucs']):
        ax.text(i, v + 0.001, f'{v:.4f}', ha='center', fontsize=7)
    ax.grid(axis='y', alpha=0.3)

fig.suptitle('Feature Selection Strategy Comparison (red = winner)', fontsize=12, fontweight='bold')
plt.tight_layout()
plt.savefig('images/feature_selection_comparison.png', dpi=150, bbox_inches='tight')
plt.show()
print('Saved: images/feature_selection_comparison.png')
```

---

*გრაფიკების გენერირების შემდეგ Kaggle-ზე `images/` ფოლდერი გადმოწერე და GitHub-ზე ატვირთე, შემდეგ README სურათები ასე ჩადე:*
```markdown
![Bias-Variance](images/bias_variance_comparison.png)
![Model Comparison](images/model_auc_comparison.png)
![Feature Importance](images/feature_importance_xgboost.png)
![Feature Selection](images/feature_selection_comparison.png)
```
